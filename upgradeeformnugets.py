...
def main():
    ...
    if int(get_git_status()) > 0:
        ...
        os.chdir("ServiceItemsPlanningPlugin")
        
        current_number_of_commits = get_commit_count()
        packages = [
            'Microting.eForm', 'Microting.eFormApi.BasePn', 'Microting.ItemsPlanningBase'
        ]
        process_repository('ServiceItemsPlanningPlugin.csproj', packages, 'eform-service-items-planning-plugin')
        ...
        if new_number_of_commits > current_number_of_commits:
            ...
            os.chdir("..")
            run_command(f"github_changelog_generator -u microting -p eform-service-items-planning-plugin -t {os.getenv('CHANGELOG_GITHUB_TOKEN')}", shell=True)
